...
for (const m of ['listModels','refreshCloudModels','listCommands','fetchCommands','refreshCommands','chat','designGenerate','stop','steer','clear','pickFolder','ensureWorkspace','loadState','saveState','lanServer','lanConnect','lanConnectDevice','lanDisconnect','workspaceUpsert','workspaceSeed','lanRefresh','lanRequestDeviceUpdate','selectUpdateInstaller','offerUpdate','requestUpdate','respondUpdateRequest','acceptUpdateOffer','openUpdateInstaller','checkAppUpdate','downloadAppUpdate','appInfo','installDependencies','terminalOpen','windowControl','browserShow','browserHide','browserNavigate','browserAction']) {
  window.ollama[m] = (...a) => Promise.resolve(
    ...
    : m === 'chat' ? { ok: true }
    : m === 'designGenerate' ? { ok: false, error: 'Design generation is unavailable in the synthetic stream preview.' }
    : {}
  );
}
